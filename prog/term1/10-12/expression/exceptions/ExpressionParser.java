...
public class ExpressionParser implements CharSource, Parser {
    ...
    public TripleExpression parse(String expression) throws ParsingException, OverflowException {
        ...
        line = expression;
        nulling();
        TripleExpression temp = parseExpFunc();
    ...
    private TripleExpression parseExpFunc() throws ParsingException, OverflowException {
        skipWhiteSpaces();
    ...
    public void parseSomeOperation(char kindOfOperation) throws ParsingException, OverflowException {
        ...
        if (kindOfOperation == 'a') {
            lastExpFunc = new CheckedAbs(parseExpFunc());
        } else if (kindOfOperation == 's') {
            lastExpFunc = new CheckedSqrt(parseExpFunc());
        }
    ...
    private void parseUnaryMines() throws ParsingException, OverflowException {
        ...
        if (Character.isDigit(ch)) {
            ...
            position--;
            lastExpFunc = new CheckedNegate(parseExpFunc());
        }
        prevOp = temp;
        skipWhiteSpaces();
    }
